...
class DisconnectFromSlackTest extends AbstractEntrypointTest
{
    ...
    protected function setUp(): void
    {
        ...
        $this->tribe->setSlackStatus(Tribe::SLACK_AUTHORIZED);
    ...
    {
        $url = sprintf('/api/tribe/%s/disconnectSlack', $this->tribe->getId());
        $response = $this->request('POST', $url, $this->user);
        $this->assertEquals(Response::HTTP_OK, $response->getStatusCode());
        $json = json_decode($response->getContent(), true);
        $this->assertEmpty($json);
        $this->em->refresh($this->tribe);
        $this->assertEquals(Tribe::SLACK_STATUS_NOT_ASKED, $this->tribe->getSlackStatus());
    }
}