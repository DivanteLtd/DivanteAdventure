...
class ShowActionTest extends AbstractEmployeesTest
{
    ...
    {
        $url = '/api/employees/id/'.$this->employee->getId();
        $response = $this->request("GET", $url, $this->user);
        $this->assertEquals(Response::HTTP_OK, $response->getStatusCode());
        /** @var array $json */
        $json = json_decode($response->getContent(), true);
        $this->assertArrayHasKey('employee', $json);
        $this->assertArrayHasKey('contracts', $json);
        $this->assertArrayHasKey('roles', $json);
        $this->assertArrayHasKey('askForSlack', $json);
    }
}
